var wheel = document.getElementById("w"),
	...
	inputs = [["Hue", hueMax], ["Saturation", HSVmax], ["Value", HSVmax],
		["Red", RGBmax], ["Green", RGBmax], ["Blue", RGBmax], ['Opacity', 100]],
	inputIds = "HSVRGBO",
	radioIds = "YJKMQF",
...
function solveHSV(h, s, v) {
	...
	return val.map(function (x) {
		return (x + v / HSVmax - C) * RGBmax;
	});
}

function textBoxChange(target) {
	var val = target.value,
		index = inputIds.indexOf(target.id), tmp;
	if(target.id == 'O') {
		tmp = currentColor;
		opacity = val;
	} else if (index < 3) {
...
function setUpInfo() {
	var inputs = document.getElementById('color_info').getElementsByTagName('input');
...
function setUpPanels() {
	var panels = document.getElementById('panels').getElementsByTagName('li');
...
function setupInputs() {
	...
	for (var i = 0; i < inputs.length; i++) {
		...
		if(typeof radioIds[i] != 'undefined') {
...
function setCurrent(r1, g1, b1, r2, g2, b2) {
	...
	var img = document.getElementById("trans");
	var pat = currentCtx.createPattern(img, "repeat");
	currentCtx.fillStyle = pat;

	currentCtx.beginPath();
	currentCtx.arc(0, 0, size-1, -1 * Math.PI / 2, Math.PI / 2, true);
	currentCtx.arc(0, 0, size-1, Math.PI / 2, -1 * Math.PI / 2, true);
	currentCtx.closePath();
	currentCtx.fill();

	for (i = 0; i < 2; i++) {
		currentCtx.fillStyle = i ? getRGBA(r1, g1, b1, opacity) : getRGBA(r2, g2, b2, opacity);
		currentCtx.beginPath();
...
function getRGBA(r, g, b, a) {
	return 'rgba(' + Math.round(r) + ', ' + Math.round(g) + ', ' + Math.round(b) + ', ' + a/100 + ')';
}
...
function updateColor(r, g, b, input) {
	...
	for (var i = 0; i < inputBoxes.length; i++) {
		if (input != inputBoxes[i].id && inputBoxes !== document.activeElement) {
			inputBoxes[i].value = Math.floor(boxValues[i]);
		}
	}
...
function getCIELAB(r, g, b) {
	var R = ( r / 255 );        //R from 0 to 255
	var G = ( g / 255 );        //G from 0 to 255
	var B = ( b / 255 );        //B from 0 to 255
...
function updateInfo(r, g, b) {
	document.getElementById('cssRGB').value = 'rgb(' + Math.round(r) + ', ' + Math.round(g) + ', ' + Math.round(b) + ')';
	document.getElementById('cssRGBA').value = 'rgba(' + Math.round(r) + ', ' + Math.round(g) + ', ' + Math.round(b) + ', ' + opacity/100 + ')';
	var hsl = getHSL(r, g, b);
	document.getElementById('cssHSL').value = 'hsl(' + Math.round(hsl[0]) + ', ' + Math.round(hsl[1]) + '%, ' + Math.round(hsl[2]) + '%)';
	document.getElementById('cssHSLA').value = 'hsla(' + Math.round(hsl[0]) + ', ' + Math.round(hsl[1]) + '%, ' + Math.round(hsl[2]) + '%, ' + opacity/100 + ')';
	var hex = getHex(r, g, b);
	document.getElementById('cssHex').value = hex;
	var name = colorNames[hex.toLowerCase()];
	if(typeof name == 'undefined') {
		document.getElementById('cssName').value = '';
	} else {
		document.getElementById('cssName').value = name;
	}

	var RGBId = ["RGB-R", "RGB-G", "RGB-B"];
	var RGB = [r, g, b];
	for(var i = 0; i < RGBId.length; i++) {
		document.getElementById(RGBId[i]).value = Math.round(RGB[i]);
	}

	var HSLId = ["HSL-H", "HSL-S", "HSL-L"];
	for(var i = 0; i < HSLId.length; i++) {
		document.getElementById(HSLId[i]).value = Math.round(hsl[i]);
	}

	var HSVId = ["HSV-H", "HSV-S", "HSV-V"];
	var hsv = solveRGB(r, g, b);
	for(var i = 0; i < HSVId.length; i++) {
		document.getElementById(HSVId[i]).value = Math.round(hsv[i]);
	}

	var CMYKId = ["CMYK-C", "CMYK-M", "CMYK-Y", "CMYK-K"];
	var cmyk = getCMYK(r, g, b);
	for(var i = 0; i < CMYKId.length; i++) {
		document.getElementById(CMYKId[i]).value = Math.round(cmyk[i]);
	}

	var LABId = ["Lab-L", "Lab-a", "Lab-b"];
	var cielab = getCIELAB(r, g, b);
	for(var i = 0; i < LABId.length; i++) {
		document.getElementById(LABId[i]).value = Math.round(cielab[i]);
	}
}
